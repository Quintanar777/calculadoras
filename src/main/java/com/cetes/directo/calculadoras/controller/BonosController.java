...
public class BonosController {
    ...
    public static final Logger logger = LoggerFactory.getLogger(BonosController.class);
    @Autowired
    private BonosDelegate bonosDelegate;
    ...
    public String cetes(){
        return "bonos/main-bonos";
    }

    /**
     * Calcular bondes
     * */
    @RequestMapping("calc/bonos")
    public ResponseEntity<?> calcBondes(@RequestBody RequestCommonsDTO request){
        ...
        List<BonosDto> calcBonosResponse = bonosDelegate.calcBonos(request.getMonto(), request.getPlazo());
        ...
        tabla.append("<tr>");
        tabla.append("<td>Monto Bonos</td>");
        int auxIndex = 5;
        ...
        tabla.append("<tr>");
        tabla.append("<td>Rendimiento Bonos</td>");
        auxIndex = 5;
        while(auxIndex<= calcBonosResponse.size()){
            BonosDto bondes = calcBonosResponse.get(auxIndex);
            auxIndex += 6;
